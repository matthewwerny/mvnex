...
- **Canonical-version confirmation always uses `search.maven.org`**; confirming no version falls back to the mirror's highest stable version (Sonatype's endpoint can't answer gav queries).
