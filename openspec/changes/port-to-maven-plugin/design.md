...
### D9 — Port the textual POM algorithm verbatim
`PomProjectDependencyRepository` keeps the C++ semantics: regex `<dependency\b[^>]*>([\s\S]*?)</dependency>` for existing keys; insert before the last literal `    </dependencies>`, else wrap and insert before the last `</project>`. The file is read and written as **ISO-8859-1**, which maps every byte to one character and back, so any encoding (including a BOM) round-trips exactly like the C++ byte-oriented I/O; all markers and inserted text are ASCII. Inserted lines use the file's own line separator (CRLF if its first line break is CRLF, else LF). That matches C++ on Windows for CRLF files and avoids the mixed endings C++ produced on Unix; C++'s Windows side effect of rewriting LF files as CRLF is intentionally not reproduced. The repository loads existing keys once per use-case execution (the C++ re-read the file for every `containsDependency` call; behavior is identical).
...
Preserved quirks (documented, not fixed): `junit:junit` parses as term `junit` + version `junit`; pre-release check matches the substring `-m` anywhere; duplicate detection counts dependencies in `dependencyManagement`/plugins; insertion keys on the exact 4-space `    </dependencies>` text; "Search again" drops an explicit version. (The earlier quirks "coordinate-without-version uses unfiltered `latestVersion`" and "deps.dev falls back to the oldest version" are fixed by the canonical version step; see the delta table.)
...
- **Maven 4 runtime.** Maven 4 runs Maven 3 API plugins, but `requiresProject=false` + aggregator + Prompter behaviors are not verified there → add an allowed-to-fail CI job on the latest Maven 4 release candidate.
...
- **Maven Central publishing is deferred to a future change.** This is an initial port, so releases are GitHub prereleases only. Consequence for installation: according to the source of maven-install-plugin 3.2.0 (the latest stable), `InstallFileMojo` writes no plugin-prefix metadata. A released JAR installed with `mvn install:install-file` can therefore only be run by its fully qualified coordinates. To get the short `ex:` prefix, users must build and install from source (`mvn install`, where maven-plugin-plugin adds the prefix metadata) and then run `setup`. The README documents both paths, and task 11.4 verifies this claim. The namespace choice (`com.sebas3261` needs proof of domain ownership; `io.github.sebas3261` is verifiable via GitHub) is left to the Central change.
...
- **Prerequisites are declared**: `requiredJavaVersion` 17 and `requiredMavenVersion` 3.9.0, with a CI leg on Maven 3.9.0 (raise the floor if it fails).
