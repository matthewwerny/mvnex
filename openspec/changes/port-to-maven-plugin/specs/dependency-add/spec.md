...
The goal SHALL execute exactly once per invocation, even inside a multi-module reactor.
...
#### Scenario: Missing dependency list
- **WHEN** the user runs `mvn ex:add` with no `ex.deps`
